var courses = (function ($) {
	...
	self.add_category = function (msg) {
		...
		if (category && category.length > 0) {
			window.location.href = '/courses/category/add?category=' + encodeURIComponent (category);
		}
		return false;
	};
	...
	self.add_page = function (course, msg) {
		...
		if (page && page.length > 0) {
			window.location.href = '/courses/page/add?course=' + course + '&page=' + encodeURIComponent (page);
		}
		return false;
	};
	 ...
	 * Usage: courses.notice ('#saving', 'Saving...');
	 */
...
$(function () {
	 ...
	 */
	$('.category-title')
		.hover (courses.on_category, courses.off_category)
		.click (courses.edit_category);

	$('.page-rename')
		.click (courses.edit_page);

	$('.add-learner')
		.click (courses.add_learner);

	$('#learners')
		.on ('click', '.remove-learner', courses.remove_learner);

	$('.feedback-form')
		.on ('submit', courses.feedback_submit);
});
